...
public class Pattern2_7 : IPattern
{
    ...
    {
        _pattern = CSVReader.Read("Stage2/pattern7.csv");
    }
    ...
    public IEnumerator Attack()
    {
        ...
        for (int i = 0; i < _pattern.Count - 1; i++)
        {
            ...
            note.GetComponent<Note>().noteNumber = 7;
    ...
    public void Feedback()
    {
        ...
        List<GameObject> _activeNotes = Managers.Pool.GetActiveAliveNotes(7);
        float _noteDistance = _noteSpeed * (float)(AudioSettings.dspTime - _startDsp) + Managers.Game.StageStartDelay[2];

        int cnt = 0;
        for (int i = Managers.Game.curNoteInStage2[7]; i < Managers.Game.curNoteInStage2[7] + _activeNotes.Count; i++)
        {
