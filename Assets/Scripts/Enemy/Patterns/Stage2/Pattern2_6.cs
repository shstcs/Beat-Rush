...
public class Pattern2_6 : IPattern
{
    ...
    public IEnumerator Attack()
    {
        ...
        for (int i = 0; i < _pattern.Count - 1; i++)
        {
            ...
            note.GetComponent<Note>().noteNumber = 6;
    ...
    public void Feedback()
    {
        ...
        List<GameObject> _activeNotes = Managers.Pool.GetActiveAliveNotes(6);
        float _noteDistance = _noteSpeed * (float)(AudioSettings.dspTime - _startDsp) + Managers.Game.StageStartDelay[2];

        int cnt = 0;
        for (int i = Managers.Game.curNoteInStage2[6]; i < Managers.Game.curNoteInStage2[6] + _activeNotes.Count; i++)
        {
